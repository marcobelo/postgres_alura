...
SELECT (primeiro_nome || NULL || ultimo_nome) AS nome_completo
FROM aluno;
-- Usando a funcao CONCAT o NULL passa a ser ignora, no exemplo retorna: MarcoBelo
SELECT CONCAT('Marco', NULL, 'Belo');
...
SELECT TRIM(UPPER(CONCAT('potato', ' ', 'bread')) || '    ');
-- Date functions
-----------------
...
SELECT TO_CHAR(NOW(), 'DD(DAY), MM(MONTH), YYYY');
SELECT TO_CHAR(128.3111::REAL, '999D99');